...
function treeToDiagram(tree, diagramSvg, diagramGroup, options) {
  ...
  var rectWidth = 100; // Set the desired width
  var rectHeight = 100; // Set the desired height
  // Add visual elements.
  ...
  for (var levelIdx = 0; levelIdx != levels.length; levelIdx++) {
    ...
    for (var memberIdx = 0; memberIdx != level.length; memberIdx++) {
      ...
      for (var nodeIdx = 0; nodeIdx != group.length; nodeIdx++) {
        ...
        rect.setAttribute("width", rectWidth + "px"); // Set the width
        rect.setAttribute("height", rectHeight + "px"); // Set the height
  ...
  var rectElements = diagramGroup.querySelectorAll("rect");
  rectElements.forEach(function(rect) {
    rect.addEventListener("dblclick", function(event) {
      ...
      if (newText !== null) {
        ...
        var textHeight = parseFloat(window.getComputedStyle(text).fontSize);
        var x = parseFloat(rect.getAttribute("x")) + (rectWidth - textWidth) / 2;
        var y = parseFloat(rect.getAttribute("y")) + (rectHeight + textHeight) / 2;
        text.setAttribute("x", x + textWidth / 2); // Adjust x to center text
